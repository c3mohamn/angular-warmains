...
export class TooltipDirective {
  @Output()
  beforeShow: EventEmitter<TooltipDirective> = new EventEmitter<TooltipDirective>();
  @Output()
  show: EventEmitter<TooltipDirective> = new EventEmitter<TooltipDirective>();
  @Output()
  beforeHide: EventEmitter<TooltipDirective> = new EventEmitter<TooltipDirective>();
  @Output()
  ...
  public showTooltip(options: ContentOptions) {
    ...
    const componentFactory = this.componentFactoryResolver.resolveComponentFactory(HoveredContentComponent);
    this.contentCmpRef = this.viewContainerRef.createComponent(componentFactory);
    this.beforeShow.emit(this);
    this.document.querySelector('body').appendChild(this.contentCmpRef.location.nativeElement);
  ...
  private getPosition(): [number, number] {
    ...
    const scrollYOffset =
      this.window.scrollY || this.document.documentElement.scrollTop || this.document.body.scrollTop || 0;

    const scrollXOffset =
      this.window.scrollX || this.document.documentElement.scrollLeft || this.document.body.scrollLeft || 0;

    switch (this.position) {
      case 'top':
        return [scrollXOffset + left + width / 2, scrollYOffset + top - padding];
      case 'top-left':
        return [scrollXOffset + left + width, scrollYOffset + top - padding];
      case 'top-right':
        return [scrollXOffset + left, scrollYOffset + top - padding];
      case 'right':
        return [scrollXOffset + left + width + padding, scrollYOffset + top + height / 2];
      case 'left':
        return [scrollXOffset + left - padding, scrollYOffset + top + height / 2];
      case 'bottom':
        return [scrollXOffset + left + width / 2, scrollYOffset + top + height + padding];
      case 'bottom-left':
        return [scrollXOffset + left + width, scrollYOffset + top + height + padding];
      default:
        // bottom-right
