...
export class TalentCalculatorEffects {
  ...
  load$: Observable<Action> = this.actions$.pipe(
    ofType<TalentCalculatorActions.LoadTalents>(TalentCalculatorActionTypes.LOAD_TALENTS),
    // get class id
    ...
    concatMap(payload => {
      ...
      return this.talentService.getTalentDetails(classId).pipe(
        ...
        map(tooltips => new TalentCalculatorActions.LoadTalentsSuccess(tooltips)),
  ...
  addPoint$: Observable<Action> = this.actions$.pipe(
    ofType<TalentCalculatorActions.AddTalentPoint>(TalentCalculatorActionTypes.ADD_TALENT_POINT),
    map(action => action.payload),
    map(talent => {
      if (this.talentCalculatorService.canAddPoint(talent)) {
        const newMeta = this.talentCalculatorService.getUpdatedTalentMetaInfo(talent, 1);
        return new TalentCalculatorActions.AddTalentPointSuccess([talent, newMeta]);
      } else {
        return new TalentCalculatorActions.TalentError(`Cannot add point to ${talent.name}.`);
      }
    })
  );
  ...
  removePoint$: Observable<Action> = this.actions$.pipe(
    ofType<TalentCalculatorActions.RemoveTalentPoint>(TalentCalculatorActionTypes.REMOVE_TALENT_POINT),
    map(action => action.payload),
    map(talent => {
      if (this.talentCalculatorService.canRemovePoint(talent)) {
        ...
        return new TalentCalculatorActions.TalentError(`Cannot remove point from ${talent.name}.`);
      }
    })
  );
  ...
  addGlyph$: Observable<Action> = this.actions$.pipe(
    ofType<TalentCalculatorActions.AddGlyph>(TalentCalculatorActionTypes.ADD_GLYPH),
    map(action => action.payload),
    map(payload => {
      ...
      if (this.talentCalculatorService.canAddGlyph(glyph, index)) {
        ...
        const newMeta = this.talentCalculatorService.getUpdatedGlyphMetaInfo(index, glyph, true);
        return new TalentCalculatorActions.AddGlyphSuccess([newGlyphs, newMeta]);
      } else {
        return new TalentCalculatorActions.TalentError(`Cannot add glyph ${glyph.name}.`);
      }
    })
  );
  ...
  removeGlyph$: Observable<Action> = this.actions$.pipe(
    ofType<TalentCalculatorActions.RemoveGlyph>(TalentCalculatorActionTypes.REMOVE_GLPYH),
    map(action => action.payload),
    map(index => {
      const newGlyphs = this.talentCalculatorService.removeGlyph(index);
      const newMeta = this.talentCalculatorService.getUpdatedGlyphMetaInfo(index);
      return new TalentCalculatorActions.RemoveGlyphSuccess([newGlyphs, newMeta]);
    })
  );

  @Effect()
  resetTalents$: Observable<Action> = this.actions$.pipe(
    ofType<TalentCalculatorActions.ResetTalentPoints>(TalentCalculatorActionTypes.RESET_TALENT_POINTS),
    map(action => action.payload),
