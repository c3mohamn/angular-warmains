 ...
 */
export function canAddGlyph(glyphState: Glyph[], glyph: Glyph, index: number): boolean {
  ...
  const count = glyphState.reduce((total, g) => (total = g && glyph.type === g.type ? total + 1 : 0), 0);

  if (count === 3 && glyphState[index] && glyphState[index].type !== glyph.type) {
 ...
 */
export function canAddPoint(state: TalentCalculatorState, talent: Talent): boolean {
  let canAdd = true;
 ...
 */
export function canRemovePoint(state: TalentCalculatorState, talent: Talent): boolean {
  let canRemove = true;
  ...
  if (talent.row !== lastActiveRow) {
    ...
    while (lastActiveRow - i > talent.row) {
      if (sumRows(lastActiveRow - i, state.meta.treeRows[talent.tree]) <= (lastActiveRow - i) * 5) {
